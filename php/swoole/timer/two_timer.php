...
/**
 * 执行并传递参数
 * D:\code\sw\php-cli\bin/php two_timer.php -a 7 -b 9
 * D:\code\sw\php-cli\bin/sw two_timer.php -a 7 -b 9
 *
 * kill -9  pid
 * dos命令杀死进程命令
...
$process = new \Swoole\Process(function (\Swoole\Process $process) use($ship_company,$curl){
    //一分钟以后才会 发出请求
    Swoole\Timer::tick(60*1000, function (int $timer_id) use($ship_company,$process,$curl){
